...
export default function FuelQuoteItem({user}) {
    ...
    const priceCalulated = () => {
        ...
        const userQuotes = quotes.filter(quote => quote.author.name === user.name)
        ...
        const margin = (currentPrice * (locationFactor - rateHistoryFactor + gallonRequestedFactor + companyProfitFactor))
    ...
    const buttonDisable = ((quoteInput.gallons).length > 0) & ((quoteInput.delivery_date).length >0);
